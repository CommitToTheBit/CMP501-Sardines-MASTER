...
\vspace{5pt}\noindent
In the meanwhile, there are more concrete fixes to be made - consider, for instance, how travel times between client and server are calculated. Currently, \texttt{delay} is averaged from $5$ \texttt{syncPacket}s sent back and forth immediately on joining a lobby, the slow loading time made worthwhile by more accurate predictions. What this theory misses is travel times may vary over larger time scales; the experience of testing latency suggests \texttt{delay}s should be recalculated at a reasonable interval. %In theory, it is enough to average over a small sample of these times immediately on joining a lobby, but the experience of testing latency suggests this value could ebb and flow over a larger timescale. Recalculating...

\paragraph{Throttle} On one hand, \textit{Sardines} would appear to have a built-in threshold for throttle: since it only 
...
\vspace{5pt}\noindent
For all the optimisations and oversights discussed in this section, it's worth noting how fundamental testing has already been in development. With network programming being notoriously unpredictable, \textit{Sardines} has necessarily involved a lot of QA: a feature that works offline must be tested between a single client and the server; then between two clients on the same device; then across devices, or between more clients, or under poor network conditions; the list goes on. 
